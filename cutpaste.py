...
class AnatMix(CutPaste):
    ...
    def __call__(self, img):
        ...
        if np.random.random()<0.5:
            draw.ellipse((coord[0]-cut_w//2+self.margin, coord[1]-cut_h//2+self.margin, coord[0]+cut_w//2-self.margin, coord[1]+cut_h//2-self.margin), fill=mask_value)
        else:
            draw.rectangle((coord[0]-cut_w//2+self.margin, coord[1]-cut_h//2+self.margin, coord[0]+cut_w//2-self.margin, coord[1]+cut_h//2-self.margin), fill=mask_value)
